...
export class VehiclePicturesComponent {
      ...
      for (const property in this.images.nativeElement.files) {
        if( property != 'length' && property != 'item' ){
          ...
          setTimeout(() => {
            ...
            this._vehiclesService.setImage(this.vehicle_id, this.images.nativeElement.files[property])
            .subscribe({
  ...
  async deleteImages() {
    const { value: fruit } = await Swal.fire({
      ...
      input: 'select',
      inputOptions: {
        intelimotors: 'Eliminar imágenes de intelimotors',
        abcars: 'Eliminar imágenes de abcars'
      },
      ...
      inputValidator: (value) => {
        return new Promise((resolve) => {
          if (value === 'abcars') {
            this.deleteImagesToExternalWebSite('no');
          } else {
            this.deleteImagesToExternalWebSite('yes');
          }
        })
      }
    });
